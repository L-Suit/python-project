...
```bash
sudo docker pull ultralytics11/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it ultralytics11/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it -v "$(pwd)"/datasets:/usr/src/datasets ultralytics11/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it --gpus all ultralytics11/yolov5:latest
```
